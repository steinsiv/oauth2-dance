...
export const URLAuthorizeResponse = (
  url: string,
...
export const URLAuthorizationErrorResponse = (
  url: string,
  options: AuthorizationErrorResponseOptions,
): string => {
  const result = new URL(url);
  result.searchParams.append("error", options.error);
  options.error_description
    ? result.searchParams.append("error_description", options.error_description)
    : {};
  options.error_uri
    ? result.searchParams.append("error_uri", options.error_uri)
    : {};
